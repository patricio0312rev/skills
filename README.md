...
### Option 1: Use in Claude Desktop (Recommended)
...
1. **Backend Development** → `api-endpoint-generator` → `auth-module-builder` → `error-handling-standardizer`
2. **Frontend Development** → `component-scaffold-generator` → `form-wizard-builder` → `state-ux-flow-builder`
3. **DevOps** → `github-actions-pipeline-creator` → `preview-environments-builder` → `rollback-workflow-builder`
4. **AI Engineering** → `prompt-template-builder` → `rag-pipeline-builder` → `evaluation-harness`
5. **Security** → `secrets-scanner` → `auth-security-reviewer` → `threat-model-generator`
